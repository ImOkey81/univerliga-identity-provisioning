    ...
    public ProcessedEventStatus process(MockEventRequest event) {
        ...
        try {
            ...
            if (existing.isPresent()
                && existing.get().getStatus() == ProcessedEventStatus.FAILED
                && existing.get().getFailCount() >= appProperties.getProvisioning().getBroker().getMaxFailCount()) {
                log.warn("Event {} skipped due to max fail_count={}", event.eventId(), existing.get().getFailCount());
                return ProcessedEventStatus.IGNORED;
            }
