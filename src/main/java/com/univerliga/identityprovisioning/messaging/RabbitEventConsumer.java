...
public class RabbitEventConsumer {
    ...
    public void consume(Message message, Channel channel) throws Exception {
        ...
        try {
            ...
            log.error("Event {} failed after retries, sending to DLQ", event.eventId(), ex);
    ...
    private ProcessedEventStatus withRetries(MockEventRequest event) {
        int maxRetries = Math.min(appProperties.getProvisioning().getBroker().getMaxRetries(), BACKOFF_MS.length);
